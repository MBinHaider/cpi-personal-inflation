...
interface Props {
  ...
  questionNumberLabel: string;                          // e.g. "Question 1"
  title: string;
  sub?: string;
...
export function QuestionScreen({
  stepIndex, stepCount, questionNumberLabel, title, sub, children,
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-7 shadow-sm">
      ...
      <div className="flex items-center justify-between mb-6">
        {onBack ? (
          <button onClick={onBack} aria-label={t('common.back')} className="w-8 h-8 rounded-full border border-gray-200 dark:border-slate-600 flex items-center justify-center text-gray-500 hover:border-[#0066cc] hover:text-[#0066cc]">
        ...
        <span className="text-xs text-gray-400" aria-live="polite">{stepIndex + 1} / {stepCount}</span>
      </div>
